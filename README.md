...
## How it works
The game is made in Godot 4.7 using GDScript. The game is built in Godot 4.4 using GDScript. One of the core focuses was implementing custom physics controllers and object interaction scripts so the enemy pathfinding feels natural without ships clumping together. The camera boundary systems also dynamically adjust if the player selects an upgrade that zooms out the view. 
